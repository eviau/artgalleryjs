...
var guard_selected;
var isguardselected = false;
var ismoving = false;

var changedirection = false;
var isguardselected_dir = false;
...
function updateSector(index) {
    ...
    var sector = scene.observers[index].sector
    var observer = scene.observers[index]
    scene.observers[index].sector.centre = scene.observers[index].loc;
    // console.log(scene.observers[index].loc)
    scene.observers[index].sector.midDir = scene.observers[index].dir;
...
function update(index) {
    ...
    var fov = scene.observers[index].fov;
...
function render() {
    ...
    ctx.fillStyle = 'white';
    ctx.fillRect(20, 20, ctx.width-40, ctx.height-40);
    ...
    // draw personnel
    for (var i = 0; i < scene.observers.length; i++) {
        drawPerson(scene.observers[i]);
        // var fov = s;
        // var sector = ;
        drawFoV(scene.observers[i].fov, scene.observers[i].sector);
    }


    if (debug) {
        var radius = visionRadius;
        var dir = scene.observers[0].dir;
        var pos = scene.observers[0].loc;
...
function mainLoop() {
    var index;
    for (index = 0; index < scene.observers.length; ++index) {
        update(index);
        render();
    }
...
function start() {
    ...
    canvas.addEventListener('keydown', keyEvents, false);
    canvas.addEventListener('keyup', keyEvents, false);
...
function keyEvents(e) {
    ...
    if (keysdict[e.keyCode] !== undefined) { // check if its a key we are listening for
        keysdict[e.keyCode] = (event.type === "keydown"); // set the state up or down
...
function handleClick(e) {
    // scene.observers[0].loc = getCursorPosition(e, canvas);
    var curpos = getCursorPosition(e, canvas);
    var index
    if (changedirection === false) {
        for (index = 0; index < scene.observers.length; index++) {
            // si le clique est sur un garde, alors on fait onguard = index_du_garde
            pos_guard = scene.observers[index].loc;
            if (Math.sqrt((pos_guard[0] - curpos[0]) * (pos_guard[0] - curpos[0]) + (pos_guard[1] - curpos[1]) * (pos_guard[1] - curpos[1])) < personRadius) {
                guard_selected = index;
...
function handleMouseMove(e) {
    ...
    // console.log(isguardselected);
    ismoving = true;
    if (isguardselected === true && isguardselected_dir === false) {
        ismoving = false;
        // console.log('is moving');
        index = guard_selected;
        scene.observers[index].loc = getCursorPosition(e, canvas);

        window.requestAnimationFrame(mainLoop);


    }
    // console.log('WE HARE HERRERERERERERE');
    // console.log(keysdict.KeyD);
    if (e.shiftKey) {
        index = guard_selected;
        setDirection(scene.observers[index], lookAt);
...
function handleMousedown(e) {
    // console.log('isguardselected', isguardselected)
    if (ismoving === false) {
...
function addguard() {
    // console.log('hello')
    var radius_2 = visionRadius * visionRadius;
    scene.observers.push({
        loc: vec2.fromValues(374, 203),
        dir: vec2.fromValues(-0.707106781186, 0.707106781186),
        colour: "255,0,0"
    })
    scene.observers[scene.observers.length - 1].sector = {
        centre: vec2.fromValues(374, 203),
        radius: visionRadius,
        radius_2: radius_2,
        fovEdges: [{
            vec: vec2.create(),
    ...
    scene.observers[scene.observers.length - 1].fov = {
        blockingEdges: [],
        anglePtSet: new Set(),
        anglePoints: [],
        hitPoints: []
...
function checkGameO() {
    ...
    pixelsarecovered = true;

    var imgData = ctx.getImageData(0, 0, canvas.width, canvas.height);
    var imgData2 = ctx.getImageData(0, 0, canvas.width, canvas.height);

    console.log('the gallery might be covered...');
    // invert colors
    var i;
    for (i = 0; i < imgData.data.length; i += 4) {
        // console.log("checking one pixel");
        // console.log(i);
        ...
        if (imgData.data[i] == 255 && imgData.data[i + 1] == 255 && imgData.data[i + 2] == 255 ) {
            ...
            // console.log(imgData.data[i]);
            imgData2.data[i] = 236; 
            imgData2.data[i + 1] = 245;
            imgData2.data[i + 2] = 11;
            imgData2.data[i + 3] = 215;
            // console.log(imgData2.data[i+1]);

            pixelsarecovered = false;
            // console.log('The Gallery is not covered!');
            // fillStyle = "pink"
            // break;
        }
    }
    ...
    console.log(pixelsarecovered);

    return pixelsarecovered

}

function displayGallery() {
    // if (pixelsarecovered === true){
    fillStyle = "white";
    window.requestAnimationFrame(mainLoop);
